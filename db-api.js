// ═══════════════════════════════════════════════════════════════════════════
// NSE TradeAI · Database + API Extension
// Import this into bot-server.js and call initDB() on startup
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// WRITE HELPERS — call these from bot-server.js
// ═══════════════════════════════════════════════════════════════════════════
...
export async function logTrade(trade) {
  ...
  const r = await db.query(q, [
    trade.id, trade.signalId || null, trade.symbol, trade.mode, trade.signal, trade.orderType, trade.orderId || null,
...
export function startAPI(port = 3000) {
  ...
  app.post("/api/control/halt", auth, async (req, res) => {
    ...
    await logEvent("HALT", "Bot halted from mobile app");
    res.json({ ok: true });
  });
  ...
  app.post("/api/control/resume", auth, async (req, res) => {
    ...
    await logEvent("RESUME", "Bot resumed from mobile app");
    res.json({ ok: true });
  });
